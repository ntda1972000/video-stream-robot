...
from controllable_components.pin_config import MotorPinSet


class L298NFourMotorController:
    """Controls 4 DC motors via L298N-compatible direction and enable pins."""
    ...
    def _set_single_motor(self, motor_index: int, speed: float) -> None:
    ...
    def drive(self, x: float, y: float) -> None:
        """Differential drive mapping from joystick axes (-1..1)."""
        ...
        self._set_single_motor(0, left_speed)
        self._set_single_motor(2, left_speed)
        self._set_single_motor(1, right_speed)
        self._set_single_motor(3, right_speed)

    def stop_all(self) -> None:
        for index in range(len(self._motor_pins)):
            self._set_single_motor(index, 0.0)
